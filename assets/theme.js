  ...
  theme.Product = (function() {
    ...
    function Product(container) {
      ...
      this.selectors = {
        ...
        stickyAtc: '[data-product-sticky-atc]',
        stickyAtcButton: '[data-sticky-atc-button]',
        stickyAtcText: '[data-sticky-atc-text]',
        stickyAtcPrice: '[data-sticky-atc-price]'
      };
    ...
    Product.prototype = Object.assign({}, Product.prototype, {
      ...
      cacheElements: function() {
        this.cache = {
          ...
          stickyAtc: this.container.querySelector(this.selectors.stickyAtc),
          stickyAtcButton: this.container.querySelector(this.selectors.stickyAtcButton),
          stickyAtcText: this.container.querySelector(this.selectors.stickyAtcText),
          stickyAtcPrice: this.container.querySelector(this.selectors.stickyAtcPrice),
      ...
      initStickyAtc: function() {
        if (!this.cache.stickyAtc || !this.cache.stickyAtcButton) {
          return;
        }

        var body = document.body;
        var stickyBar = this.cache.stickyAtc;
        var stickyButton = this.cache.stickyAtcButton;
        var stickyText = this.cache.stickyAtcText;
        var stickyPrice = this.cache.stickyAtcPrice;
        var addToCartButton = this.container.querySelector(this.selectors.addToCart);
        var hasOnlyDefaultVariant = stickyBar.dataset.hasOnlyDefaultVariant === 'true';
        var mobileQuery = window.matchMedia('(max-width: 749px)');
        var isMobile = function() {
          return mobileQuery.matches;
        };
        var isSealedProduct = this.container.classList.contains('product-section--sealed-pokemon');
        var bisSelector = '[data-a11y-identifier^="bis-button-"] button';

        var getBisButton = function() {
          var scope = this.cache.form || this.container;
          if (!scope) {
            return null;
          }
          var button = scope.querySelector(bisSelector);
        ...
        var getBisLabel = function(button) {
          if (!button) {
            return '';
          }
          var label = button.textContent || button.innerText || '';
          return label.replace(/\s+/g, ' ').trim();
        };

        var updateStickyState = function(variant) {
          var currentVariant = variant || (this.variants ? this.variants.currentVariant : null);
          var bisButton = null;

          if (stickyPrice && currentVariant) {
            stickyPrice.textContent = theme.Currency.formatMoney(currentVariant.price, theme.settings.moneyFormat);
          }

          if (!isSealedProduct && isMobile() && currentVariant && !currentVariant.available) {
            bisButton = getBisButton();
          }

          if (bisButton) {
            stickyButton.removeAttribute('disabled');
            if (stickyText) {
              stickyText.textContent = getBisLabel(bisButton) || 'Notify me when back in stock';
            }
            return;
          }

          if (hasOnlyDefaultVariant && currentVariant && currentVariant.available) {
            stickyButton.removeAttribute('disabled');
            if (stickyText) {
              stickyText.textContent = stickyButton.dataset.stickyAtcAddText;
            }
            return;
          }

          if (!isSealedProduct && isMobile() && hasOnlyDefaultVariant && currentVariant && !currentVariant.available) {
            stickyButton.setAttribute('disabled', 'disabled');
            if (stickyText) {
              stickyText.textContent = theme.strings.soldOut || 'Sold out';
            }
            return;
          }
          ...
          if (!hasOnlyDefaultVariant) {
            if (!currentVariant) {
              stickyButton.removeAttribute('disabled');
              if (stickyText) {
                stickyText.textContent = stickyButton.dataset.stickyAtcSelectText;
              }
              return;
            }
        ...
        var stickySyncHandle = null;
        var scheduleStickySync = function() {
          if (stickySyncHandle) {
            return;
          }
          stickySyncHandle = requestAnimationFrame(function() {
            stickySyncHandle = null;
            updateStickyState();
          });
        };
        ...
        stickyButton.addEventListener('click', function() {
          var currentVariant = this.variants ? this.variants.currentVariant : null;
          var bisButton = null;
          ...
          var variantWrapper = this.container.querySelector(selectors.variantType) || this.container.querySelector('.product-single__form');
          if (variantWrapper) {
            variantWrapper.scrollIntoView({ behavior: 'smooth', block: 'center' });
          }
        }.bind(this));

        if (!isSealedProduct && this.cache.form && 'MutationObserver' in window) {
          ...
          stickyObserver.observe(this.cache.form, { childList: true, subtree: true, attributes: true, characterData: true });
        }
        ...
        if (addToCartButton) {
          if ('IntersectionObserver' in window) {
            var observer = new IntersectionObserver(function(entries) {
              entries.forEach(function(entry) {
                if (entry.isIntersecting) {
                  ...
                  body.classList.remove('has-sticky-atc');
                } else {
                  stickyBar.classList.add('product-sticky-atc--visible');
                  body.classList.add('has-sticky-atc');
                }
              });
            }, { threshold: 0 });
            observer.observe(addToCartButton);
          } else {
            var toggleStickyFallback = function() {
            ...
            window.addEventListener('scroll', toggleStickyFallback);
            window.addEventListener('resize', toggleStickyFallback);
            toggleStickyFallback();
          }
        }

        this.container.on('variantChange' + this.settings.namespace, function(evt) {
          updateStickyState(evt.detail.variant);
        });
      },
